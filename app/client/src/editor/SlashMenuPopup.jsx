...
import { slashBridge } from './SlashCommand.js'

// React popup for the slash menu. State + key events arrive via slashBridge; the
// suggestion plugin owns the trigger/positioning data (clientRect at the caret).
export default function SlashMenuPopup() {
  ...
  useEffect(() => slashBridge.subscribe((next) => { setSt(next); setSel(0) }), [])

  useEffect(() => {
    slashBridge.setKeyHandler((e) => {
    ...
    return () => slashBridge.setKeyHandler(null)
  }, [])
  ...
  return createPortal(
    <div className="slash-menu" style={{ position: 'fixed', left: Math.round(rect.left), top: Math.round(rect.bottom + 6) }} role="listbox">
      {st.items.map((it, i) => (
        <button
          key={it.id} type="button" role="option" aria-selected={i === sel}
          className={`slash-item${i === sel ? ' sel' : ''}`}
          onMouseEnter={() => setSel(i)} onMouseDown={(e) => { e.preventDefault(); pick(i) }}
        >
          <span className="slash-item-title">{it.title}</span>
          <span className="slash-item-group">{it.group}</span>
        </button>
      ))}
    </div>,
    document.body,
