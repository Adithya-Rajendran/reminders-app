...
export default function TaskListWidget({ projectId }) {
  ...
  const { tasks, state, load, onToggle, onSetDue, onSetPriority, undo, dismissUndo } = useTaskList(loader)
    ...
    <div className="tasklist">
      ...
      {state === 'ready' && (tasks.length === 0
        ...
        : <div className="task-stream">
            {tasks.map((t) => (
              <TaskRow key={t.id} task={t} onToggle={onToggle} onSetDue={onSetDue} onSetPriority={onSetPriority} />
            ))}
          </div>)}
