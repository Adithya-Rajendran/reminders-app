...
export async function initCaldavDb() {
  ...
  // Stable integer id (= project_id for the CalDAV task store) + calendar color +
  // VTODO capability. Added idempotently so existing rows get an id assigned.
  await pool.query(`ALTER TABLE caldav_lists
    ADD COLUMN IF NOT EXISTS id BIGINT GENERATED BY DEFAULT AS IDENTITY,
    ADD COLUMN IF NOT EXISTS color text,
    ADD COLUMN IF NOT EXISTS supports_vtodo boolean NOT NULL DEFAULT true`)
  await pool.query('CREATE UNIQUE INDEX IF NOT EXISTS caldav_lists_id_uidx ON caldav_lists(id)')
...
// Lists with their stable integer id (= project_id) AND their account creds
// (so the task store can fan out reads/writes without a second query per list).
export async function listsWithId(userId) {
  const r = await pool.query(
    `SELECT l.id, l.url, l.display_name, l.color, l.supports_vtodo, l.enabled,
     ...
     WHERE a.user_id = $1 ORDER BY l.id`, [userId])
  return r.rows
}
...
export async function getListById(userId, listId) {
  ...
  const x = r.rows[0]
  return {
    list: { id: x.list_id, url: x.list_url, displayName: x.display_name, color: x.color, supportsVtodo: x.supports_vtodo, enabled: x.enabled },
    ...
    await pool.query(
      `INSERT INTO caldav_lists (account_id, url, display_name, color, supports_vtodo, enabled)
       ...
       ON CONFLICT (account_id, url) DO UPDATE SET display_name=EXCLUDED.display_name,
         color=EXCLUDED.color, supports_vtodo=EXCLUDED.supports_vtodo`,
      [acc.id, c.url, name, color, supportsVtodo],
    )
  }
