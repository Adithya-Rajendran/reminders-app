...
| **3 — co-locate styles** | Move per-widget classes to co-located `.css` (Calendar → Notes → Reminders → Upcoming), one widget per PR; shared primitives + theme vars stay global. | `widgets/<name>/<name>.css`, trimmed `styles.css` | build, per-widget e2e, manual light/dark |
