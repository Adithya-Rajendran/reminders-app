...
| **3 — co-locate styles** | **Mostly done.** Cues, Review, Frog (folded into Prioritize — `widgets/triage/TriageWidget.css`), Calendar, and Notes each got their per-widget classes (`.flow-*`/`.rv-*`/`.eq-*`/`.cal-*`/`.notes-*`/`.tree-*`) moved out of `styles.css` one PR at a time; Daily/Focus/Inbox/NotePin/Overview shipped with co-located CSS from the start. Reminders is intentionally partial: only its private classes live in `RemindersWidget.css`, while `.rem-*`/`.reminding` stay global because Daily/Upcoming/Focus and the widget-sdk `TaskRow`/`GroupPicker` also render them. Shared primitives (`.tasklist`, `.check-btn`, `.due-chip`, `.habit-*`, etc.) stay global by design. Remaining gap: Upcoming has no `UpcomingWidget.css` yet — its `.up-overdue`/`.up-filter`/`.qw-badge`/`.qw-filter` still sit in the `styles.css` tail. | `widgets/<name>/<name>.css`, trimmed `styles.css` | build, per-widget e2e, manual light/dark |
